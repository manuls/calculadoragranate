...
La portada de Calculadora Granate y `/pronosticos` usan `getForecastReport`, que obtiene la competición desde una única caché de datos. BDFutbol es la primera fuente y AS la alternativa; Redis completa resultados oficiales todavía ausentes en esas fuentes. `lib/data.ts` sigue siendo la copia inicial común.
